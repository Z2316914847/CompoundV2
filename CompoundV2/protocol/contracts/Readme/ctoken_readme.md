...
- Comp代币分发(Comp代币时治理代币), 
  ...
  - 分发 Comp治理代币逻辑
    - 每区块产生 0.5 COMP，每天约总发放: 2,880 COMP => 每秒发放: 0.0333 COMP
    - 分配规则: 
...
## 以下介绍 健康度检查 和 COMP治理代币(Supply、Borrow) 
上面 六大模块 我没有详细了解 健康度检查 和 COMP治理代币 这两大功能，下面将为你介绍。
### 健康度检查 - ComptrollerG7.getHypotheticalAccountLiquidityInternal
首先你得先想明白：为什么要 执行健康度检查？什么情况下执行健康度检查？
- 为什么要 执行健康度检查：健康度代表的是用户账户是否健康，账户监健康的话什么也不做，账户不健康的话，就会得到惩罚，所有用户要尽量保障自己的账户处于健康的状态，避免得到惩罚。执行健康度检查是为了 即时了解 自己账户健康度情况。
- 什么情况下执行健康度检查：涉及到账户资金减少、账户借贷增加：转账、赎回、借贷、清算。有人会问，为什么清算的时候要执行健康度检查？答因为清算的时候要判断短缺资金，如果用户不缺 短缺资金的话，用户就不会被清算，如果用户缺 短缺资金，用户就会被清算。
##### ComptrollerG7.getHypotheticalAccountLiquidityInternal
注意：某个市场下用户**抵押**价值 = 用户拥有的 Ctoken * 兑换率 * 每个底层资产代币价格。  某个市场下用户**借贷**价值 = 用户拥有的 Ctoken * 兑换率 * 每个底层资产代币价格。
- 1、获取用户 加入市场的资产
- 2、遍历用户 加入市场的资产
- 3、获取 用户单个市场抵押资产价值 和 用户单个市场借贷价值
- 4、求和 计算出用户加入市场的所有 质押价值和借贷价值
- 5、因为用户进行了 转账、赎回、借贷操作，所有会增加借贷、减少抵押价值（这里都归为借贷中）
- 6、判断 借贷价值 是否大于 最大质押率价值 

### COMP治理代币
首先的简单明白：每个区块产生固定数量的comp代币（）。
- 1、向 存款人和借款人 发送Comp代币奖励。
  - 以此增加流动性参与度
  - 去中性化治理
  - 价值捕获: 协议越成功 -> COMP治理代币 需求高 -> COMP治理代笔 价值推高
- 2、COMP产生与分配
  - 产生：每区块产生0.5 COMP，每天约总发放: 2,880 COMP => 每秒发放: 0.0333 COMP
  - 分配: 存款和借贷人各分配 50%
  - 各市场独立分配：管理员会为每个市场设置一个参数(CompSpeeds)，这个参数的意思是：代表这个市场在一个区块得到 x个comp。所有市场 参数CompSpeeds 相加求和 等于 0.5(每个区块参数0.5)  。比如有两个市场cUSDC和cDAI，他们的CompSpeeds分别为0.4和0.1。
    - 100区块：cUSDC市场可以获得40个comp，cDAI市场可以获得10comp、
    - 110区块：cUSDC市场可以获得44个comp，cDAI市场可以获得11comp、
- 3、用户应得 COMP = 用户持有 cToken × (当前指数 - 用户上次快照指数)
- 4、COMP治理代币 分发时机
  - mint()        - 存款
  - redeem()      - 取款
  - borrow()      - 借款
...
##### supply
```
COMP分配采用累计计算奖励：类型与sushi的奖励分发形式。
1、现在要求你回忆出sushi奖励分发
2、在COMP中有全局供应指数和一个用户级别的供应指数，用户界别的供应指数只能由用户更新，而全局供应指数所有人都能更新。
举例：
区块 1000：初始化(市场ctoken供应量=10000、每个区块发100)
  全局指数：index = 0
  用户 A：存入 1000 个 cToken（此时快照：supplierIndex = 0）
  用户 B：存入 2000 个 cToken（此时快照：supplierIndex = 0）
区块 1100：第 1 次更新（经过 100 个区块）
  deltaBlocks = 1100 - 1000 = 100 个区块
  compAccrued = 100 × 100 = 10,000 个 COMP（总奖励）
  ratio = comp奖励/总ctoken供应量 = 10,000 ÷ 10,000 = 1.0（每个代币的新增比率）
  index = 0 + 1.0 = 1.0（全局累积指数=1、但是用户级别的供应指数没有变）
区块 1200：第 2 次更新（再经过 100 个区块，这时有新用户存入ctoken，ctoken供应量增加到15000）
  deltaBlocks = 1200 - 1100 = 100 个区块
  compAccrued = 100 × 100 = 10,000 个 COMP（总奖励）
  ratio = 10,000 ÷ 15,000 = 0.6667（每个代币的新增比率）
  index = 1.0 + 0.6667 = 1.6667（累积指数，用户级别的供应指数没有变）
区块 1250：用户 A 领取奖励
  用户 A 在第 1 次更新后快照是 0，现在领取：
  supplyIndex = 1.6667（当前全局指数）
  supplierIndex = 0（用户 A 上次快照）
  deltaIndex = 1.6667 - 0 = 1.6667（指数增长）
  supplierTokens = 1000（用户 A 持有 1000 个代币）
  supplierDelta = 1000 × 1.6667 = 1,666.7（用户领取到的COMP奖励）
  用户 A 的新快照：supplierIndex = 1.6667
....等等
```
- updateCompSupplyIndex()：更新 该市场的 COMP供应状态(包括全局指数和最新区块)
我相信你看过上面的例子后，就会了解updateCompSupplyIndex()的用发了

- distributeSupplierComp()：发 COMP 奖励
我相信你看过上面的例子后，就会了解updateCompSupplyIndex()的用发了
##### borrows
borrow同理
